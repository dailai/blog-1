...
KafkaProducer负责将消息序列化，并且确定消息发往哪个节点。它支持在发送的时候，定义钩子函数。钩子函数由ProducerInterceptors表示，目前支持发送前的onSend函数，和发送错误的onSendError函数。

KafkaProducer有两种发送消息的方法，一种是不指定callback，一种是指定callback。
...
上面有个比较重要的partition方法，确定消息发往哪个分区。kafka有个默认的分区器，同时它也支持自定义。
...
Sender线程会首先调用RecordAccumulator的ready方法，来查看哪些节点的请求需要发送。
...
Sender实现了Runnable接口，它允许在一个单独的线程。它会循环的从RecordAccumulator获取消息，并且通过NetworkClient发送消息。

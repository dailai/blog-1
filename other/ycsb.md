...
工作线程 ClientThread 会根据配置的并发数，来完成指定的请求数。它读写数据库的实现，由三个组件完成：

DB 类负责实现底层的数据库操作，支持增删改查等操作

DBWrapper 类包装了 DB，提供了记录请求时间的功能

Workload 类实现了按照指定概率，来构建请求
...
在进一步介绍 CoreWorkload 之前，需要了解一下数据生成器。因为 CoreWorkload 会用到它来生成数据，构建请求
...
ycsb 支持三种类型的数据生成，数字类型，字符串类型，时间戳类型。下面来看看它的实现原理
...
上面测试模式的 operationchooser，负责生成操作类型。它的生成概率，是由配置文件指定的。Workload 在构建请求时，会根据配置，合理的限制各个请求类型数量。比如以 workloada 配置文件为例，定义了各种概率
...
上面介绍完了整个请求的发送流程，这些请求的响应时间都会被汇集起来做统计计算。Measurements 类负责统计计算，支持最大最小值，平均值，tp1，tp50，tp99，tp95等指标。Measurements 支持不同方式的统计方法，这些统计方法由 OneMeasurement 和其子类表示，下面依次介绍



### 原始统计

它的很简单，就是记录所有的数据值。然后将其排序，计算出对应的指标。
...
HDR直方图，它的变量间隔是不固定的。间隔大小开始通过指数级别增长，到了一定大小后，使用线性增长。它比起线性直方图，可以减少内存。它同样存在一定误差，误差大小为线性增长的单位。



### 分段统计

这种方式统计，只能统计最大最小值，平均值，而不能统计 tp99 等值。它将整个测试时间切分为多个小段，支持这些小段时间的分别统计，适合观察整个测试过程的变化。
...
JSON 格式，分为两种。一种是每个指标都是一个完整的 json。另一种是根节点是 array 类型的，里面的每个元素就是每个指标。
